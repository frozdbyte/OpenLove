	...
	<div class="space-y-6 pb-4">
		...
		<section class="space-y-3">
			<div class="flex items-center justify-between">
				<span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Special Milestones</span>
			...
			{#if isAddingMilestone}
				<div class="p-3 rounded-2xl bg-card border border-border space-y-2">
					<Input placeholder="Milestone Name (e.g. First Date)" bind:value={newMilestoneTitle} />
			...
			<div class="space-y-2 max-h-44 overflow-y-auto pr-1">
				{#each milestones.slice(0, 10) as m}
					<div class="flex items-center justify-between p-2.5 rounded-xl bg-card/60 border border-border/40 text-xs">
						<div class="flex items-center gap-2">
							<Trophy class="h-3.5 w-3.5 {m.isAchieved ? 'text-amber-500' : 'text-muted-foreground'}" />
